        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
          <div className="max-w-2xl">
            ...
            <h2 className="font-heading font-bold text-3xl md:text-4xl lg:text-5xl mb-4 text-[#1F2937]">
              Projetos <span className="text-[#D97706]">Metalúrgicos</span>
            </h2>
        ...
        <div className="relative">
          {isScrollable && (
            <>
              <button 
                onClick={() => scroll('left')}
                className={`absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white text-[#D97706] rounded-full p-2 shadow-md -ml-4 transition-opacity duration-300 ${
              ...
              <button 
                ...
                className={`absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white text-[#D97706] rounded-full p-2 shadow-md -mr-4 transition-opacity duration-300 ${
          ...
          >
            {filteredProjects.map((project, index) => (
              <div 
                key={project.id} 
                className={`bg-white rounded-xl overflow-hidden shadow-lg border border-gray-200 group transition-all duration-300 hover:shadow-xl flex-shrink-0 snap-start hover-lift ${
                  index === 0 
              ...
              >
                <div className="relative overflow-hidden aspect-[4/3]">
                  ...
                  {index === 0 && (
                    <div className="absolute top-4 left-4 bg-[#D97706] text-white text-xs font-medium px-3 py-1 rounded-full flex items-center gap-1 shadow-sm">
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <span className="text-xs font-medium text-[#D97706]/80 uppercase tracking-wider">
                        {project.category}
                      </span>
                      <h3 className="font-heading font-bold text-xl text-[#1F2937] mt-1 group-hover:text-[#D97706] transition-colors duration-300">
                        {project.title}
                      </h3>
                    </div>
                  </div>
